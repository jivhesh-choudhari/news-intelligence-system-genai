...
def fusion(BM25_results:np.ndarray, FAISS_results:np.ndarray, verbose=False)->List[Tuple[int, float]]:
    doc_scores = []
    ...
    FAISS_results[:, 1] = 1 - (FAISS_results[:, 1] / 2.0)  # Absolute normalization: L2 on unit vectors is bounded [0, 2]
    
    if verbose:
        print("Normalized BM25 Results:")
        print(BM25_results)
        print("Normalized FAISS Results:")
        print(FAISS_results)
    ...
    if verbose:
        print(f"Variance FAISS: {variance_faiss}, Variance BM25: {variance_bm25}, Alpha: {alpha}")
    ...
    for doc_id in all_doc_ids:
        ...
        if verbose:
            print(f"Doc ID: {doc_id}, BM25 Score: {bm25_score}, FAISS Score: {faiss_score}, Combined Score: {combined_score}")
